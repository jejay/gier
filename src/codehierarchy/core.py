"""Recognize the code-block structure of a Python file.

For every compound statement ("block") in the source this produces its
declaration (keyword plus name for ``def``/``class``, without parameters or
their brackets), the 1-based position of the *first* character of the
declaration keyword, and the 1-based position of the *last* character of the
last line of code contained in that block.
...
Blocks are ordered by their position in the source and separated by a relative
indentation marker:

* ``>``  the next block is a child of the previous one (one level deeper)
* ``|``  the next block is a sibling of the previous one (same level)
* ``<``  the next block is at a higher level than the previous one; the number
         of ``<`` characters equals the number of hierarchy levels ascended.
...
# Compound statements we treat as blocks.
...
def analyze(source: str) -> str:
    ...
    blocks = sorted(raw, key=lambda r: (r[0], r[1]))

    parts: list[str] = []
    prev_level: int | None = None
    for sl, sc, el, ec, decl in blocks:
        level = level_of[sc]
        ...
        parts.append(f"{marker}{level}:{decl}{{{sl},{sc + 1}~{el},{ec}}}")
        prev_level = level
